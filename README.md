...
3. poetry run systemd_mcp_cli --max-tool-steps 50 --model qwen3:4b "get all the service, find which one are failing and report here the reason of the failure"
or 
4. poetry run systemd_mcp_cli --max-tool-steps 50 --model qwen2.5-coder:14b "get all the service, find which one are failing and report here the reason of the failure"
